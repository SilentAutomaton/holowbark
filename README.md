...
2. На вкладке **WG** импортируйте `.conf`-файл WireGuard (см. раздел «Настройка сервера» ниже).
3. На вкладке **Peers** выберите страну, ближайшую к вашему устройству, и добавьте не менее 10 пиров — чем больше, тем устойчивее оверлей.
4. Вернитесь на главный экран и нажмите **Connect**.

Holowbark сначала поднимает оверлей Yggdrasil через выбранные пиры, а затем направляет весь трафик через WireGuard-туннель поверх него.
